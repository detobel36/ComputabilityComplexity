...
\section{Chapitre 8: SPACE Complexity}
  ...
  \subsection{Théorème de Savitch}
    ...
    \begin{theoremeBox}
      ...
      $NSPACE(f(n)) \subseteq SPACE(f^2(n))$, où $f^2(n) = (f(n))^2$, à ne pas confondre avec $f(f(n))$.  % @detobel36 Dans ce cas-ci, f^2(n) veut dire f(f(n)) ou (f(n))^2 ?
    \end{theoremeBox}
    ...
    $$CANYIELD(c_{start}, c_{accept}, 2^{df(n)}) \text{ simule } N$$  % @detobel36 C'est quoi df(n) ?
    \begin{enumerate}
